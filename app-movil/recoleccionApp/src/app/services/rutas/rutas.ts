...
export class RutasService {
  // Servicio para gestionar rutas, conectado con la API del docente
  ...
  getRutas(): Observable<{ data: Ruta[] }> {
    return this.https.get<{ data: Ruta[] }>(
      `${this.apiUrl}/rutas?perfil_id=${this.perfilUrl}`,
    );
  }
  ...
  updateRuta(id: string, ruta: Ruta): Observable<Ruta> {
    const headers = new HttpHeaders({ 'Content-Type': 'application/json' });
    return this.https.put<Ruta>(
      `${this.apiUrl}/rutas/${id}?perfil_id=${this.perfilUrl}`,
      ruta,
      { headers },
    );
  }
